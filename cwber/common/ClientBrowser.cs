...
namespace Sashulin.common
{
    public sealed class ClientBrowser : CefClient
    {
        ...
        {
            SystemLog.WriteLine("Client::OnProcessMessageReceived: SourceProcess={0}", sourceProcess);
            SystemLog.WriteLine("Message Name={0} IsValid={1} IsReadOnly={2}", message.Name, message.IsValid, message.IsReadOnly);
            ...
            for (var i = 0; i < arguments.Count; i++)
            {
                ...
                SystemLog.WriteLine("  [{0}] ({1}) = {2}", i, type, value);
            }
