...
namespace Sashulin.common
{
    ...
    class RenderProcessHandler : CefRenderProcessHandler
    {
        ...
        {
            /*缓存数据库*/
            string extensionCode = "var cachedb;" + "if(!cachedb)" + "	cachedb={};" + "(function() {"
                                   + " cachedb.Connect = function(dbName) {" + "	native function Connect(dbName);"
                                   + "	return Connect(dbName);" + " };" + " cachedb.Execute = function(commandText) {"
                                   + "	native function Execute(commandText);" + "	return Execute(commandText);" + " };"
                                   + " cachedb.Query = function(commandText) {" + "	native function Query(commandText);"
                                   + "	return Query(commandText);" + " };" + " cachedb.Close = function() {"
                                   + "	native function Close();" + "	return Close();" + " };" + "})();";
            ...
            if (w > 0 && h > 0)
            {
                string jscode = "Object.defineProperty(window.screen, 'height', {" + "    get: function() {"
                                + "        return " + h + ";" + "    }" + "});"
                                + "Object.defineProperty(window.screen, 'width', {" + "    get: function() {"
                                + "        return " + w + ";" + "    }" + "});";
            ...
            {
                webBrowser.RegistObject.GetType().GetMethods().ToList().ForEach(
                    m =>
                        {
                            if (m.IsPublic)
                            {
                                string name = string.Format("{0}", m.Name); // window.external.
                                var method = CefV8Value.CreateFunction(name, callHandler);
                                globalValue.SetValue(name, method, CefV8PropertyAttribute.None);
                            }
